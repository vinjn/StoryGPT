...
if "genre_input" not in st.session_state:
    st.session_state["genre_input"] = "Three pigs"
...
with st.sidebar:
    ...
    with st.expander("For Demo"):
        st.write(
            "Go to [this YouTube video](https://www.youtube.com/watch?v=bZG9hCiyFso)"
        )
...
def get_story_and_image(user_resp):
    ...
    if len(response_list) != 1:
        # print(response_list)
        img_prompt = response_list[0]
        sd_img = generate_image(sd_model, img_prompt, (512, 512))
    else:
        sd_img = None

    responses = list(
        filter(
            lambda x: "Stable Diffusion" not in x and "Image prompt" not in x, responses
        )
    )
    ...
    for response in responses:
        ...
        if (
            response.startswith("What")
            or response.startswith("Which")
            or response.startswith("Choose")
        ):
            label = "**" + response + "**"
        elif (
            response[1] == "."
            or response[1] == ")"
            or response[1:4] == " --"
            or response.startswith("Option")
        ):
            opts.append(response)
        else:
            story += response + "\n"

    return {"Story": story, "Radio Label": label, "Options": opts, "Image": sd_img}
...
def generate_content(story, lbl_text, opts: list, img, el_id):
    ...
    expander = st.expander(
        f"Part {story_pt}", expanded=st.session_state[f"expanded_{el_id}"]
    )
        ...
        if lbl_text and opts:
            with st.form(key=f"user_choice_{el_id}"):
                st.radio(
                    lbl_text,
                    opts,
                    disabled=st.session_state[f"radio_{el_id}_disabled"],
                    key=f"radio_{el_id}",
                )
